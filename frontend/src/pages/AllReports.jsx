...
export default function AllReports() {
    ...
    return (
      <div className="flex justify-center items-center min-h-64">
        <div className="text-center">
          ...
          <p className="text-gray-500">Memuat semua report...</p>
        </div>
      </div>
    )
  }
  ...
  const columns = [
    ...
    { key: 'mentee', label: 'Mentee', render: (mentee) => mentee?.name || '-' },
    ...
    { key: 'created_at', label: 'Tanggal', render: (date) => new Date(date).toLocaleDateString('id-ID') }
  ]
  
  const actions = (row) => (
    <button
      onClick={() => setSelectedReport(row)}
      className="text-primary hover:underline px-3 py-2 rounded-lg hover:bg-blue-50 transition-colors duration-200 font-medium"
    >
      Detail
    </button>
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-primary to-blue-600 rounded-xl p-6 text-white shadow-lg">
        <h1 className="text-3xl font-bold mb-2">Semua Report</h1>
        <p className="text-blue-100">Pantau dan kelola seluruh laporan mentee</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        {reports.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">Report akan muncul ketika mentor mulai membuat laporan</p>
          </div>
        ) : (
            ...
            <div className="space-y-3 mb-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedReport.mentor?.name || '-'}</p>
                </div>
                <div>
                  ...
                  <span className={`inline-block px-2 py-1 rounded text-xs ${
                    ...
                    {selectedReport.status === 'submitted' ? 'Terkirim' : 'Draft'}
                  </span>
                </div>
